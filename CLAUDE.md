...
Use the shared widget: `lib/core/widgets/sakal_autocomplete.dart` — `SakalAutocomplete<T>({initialValue, displayStringForOption, optionsBuilder, onSelected, decoration, enabled, style, focusNode, optionBuilder})`. Built on `RawAutocomplete`, adding Up/Down-arrow highlight navigation and Enter-to-select — capabilities Flutter's own `Autocomplete` never provides.
**History**: every picker in the app (23 files) used to hand-roll its own `Autocomplete<T>` + a copy-pasted `optionsViewBuilder`, none keyboard-navigable — a real user complaint ("I am not able to select product from product dropdown by pressing UP and Down key"). Built 2026-07-18 and migrated onto Sales Invoice's Customer + Product fields as the reference migration; the other 21 files still need migrating opportunistically, same rollout pattern as `SakalAdaptiveList`'s own history above — don't reintroduce the inline `optionsViewBuilder` pattern for a new picker.
