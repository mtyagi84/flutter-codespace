...
- **At the future Purchase Invoice** (not built yet): Dr Purchase Accrual (clears what GRN credited) + Dr Input VAT (the *real* invoiced amount) + Cr Supplier Account (the real payable) — one such entry per originating supplier (goods supplier and each charge's own supplier, e.g. a transport company, are cleared independently against their own provisional account).
