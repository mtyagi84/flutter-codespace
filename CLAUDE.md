...
- **`ril_stock_ledger.chk_stock_ledger_direction`** (036) whitelists which `trans_type` values are valid per movement direction — it anticipated `PURCHASE_RETURN`/`TRANSFER_OUT`/`ADJUSTMENT_OUT` as future outward types but not `MATERIAL_ISSUE`, so every `fn_approve_material_issue` call failed this CHECK constraint outright until migration 069 added it. **Any brand-new `trans_type` passed to `fn_post_stock_movement` must also be added to this constraint** — it's easy to miss since the failure surfaces as a generic Postgres constraint violation deep inside `fn_post_stock_movement`, not a friendly `RAISE EXCEPTION` from the calling module.
