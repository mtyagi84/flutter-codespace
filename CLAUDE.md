...
| Sales (quotations, orders, invoices, receipts, returns) | Sales Quotation (with Prospect support), Sales Order (Direct + Against-Quotation, price/discount governance, Payment Terms master), Sales Invoice / Quick Invoice (Direct + Against-Quotation/Order, first screen with real GL/stock impact, offline + Manager Review), and Credit Sales Invoice (migration 146 — same rih_sales_invoices engine, dedicated Save Draft + Approve document lifecycle, always-DEFERRED dispatch, hard future-date block, date locked after first save) done. |
| Purchase (PO, GRN, invoices, payments) | Pending |
| Inventory (stock, transfers, adjustments) | Material Requisition/Issue, Stock Transfer (Request/Transfer/Receipt), and Stock Adjustment all done. Barcode traceability + company-config field gating (Pack/Loose Qty, Barcode) audited complete app-wide. Reports: Stock Balance by Location (location-scoped), Stock Value by Location (weighted-avg cost across locations, category subtotals, dual-currency columns), Stock Details (Opening/Inward/Outward/Closing per product over a date range), Stock Ledger (per-product transaction statement with running balance, modeled on the Finance Account Ledger report, `remarks` resolved via a dedicated reusable `fn_resolve_transaction_remarks` function), Stock Transfer Register (document-register style, one row per transfer line, `v_stock_transfer_lines` view modeled on Sales Register), Pending Transfer to Receive (transfers not yet received + already-closed transfers received short, flagged via `Receipt Status`/`Pending Qty` — this schema has no true partial-receipt tracking, confirmed via migration 151's own research), Stock Receipt Register (document-register of what was actually received, `v_stock_receipt_lines`, `Short Received` shown inline on every row via the same per-line shortfall logic as Pending Transfer to Receive), and Stock Adjustment Register — TWO variants (`v_stock_adjustment_lines`), a Qty-only report auto-granted to existing users plus a separate "(with Value)" report deliberately NOT auto-granted (cost/value visibility reuses the existing per-report `ric_user_menus` permission system rather than a new column-level gate), Material Requisition Register, and Material Issue Register (Requisition has no batch/serial at all; Issue does; Department→Consumption Area is the first cascading-lookup-filter use with a plain-equality `depends_on_expand_fn=NULL` instead of a hierarchy expand function) — all eleven gated behind the engine-wide `auto_load`/"Run Report" mechanism (migration 148). Stock Details' Product filter is the first use of the generic cascading-lookup-filter engine capability itself (migration 149, `ric_report_filters.depends_on_*`) — reusable for future Sales/Inventory report filters |
| Finance (double-entry, trial balance, P&L, balance sheet, cash flow) | Payment/Receipt Voucher, Journal Voucher, Contra Voucher, and Expense Voucher (manual transaction entry, full double-entry) done. Trial Balance, Customer Ageing, Supplier Ageing, Pending Bills by Customer/Supplier, month-wise Expense Report, Profit & Loss (Summary + Account Detail), Balance Sheet (Summary + Account Detail), and Cash Flow Statement (Summary + Account Detail, Direct method) all done — all four core financial statements complete |
...
**Real regression, fixed 2026-08-06**: the first version never cleared the header on `dispose()` at all (to dodge a different race with the pop-reveal `RouteObserver` hook above), which left the TopBar stuck showing a screen's title forever after navigating away via `context.go()` to a screen that doesn't use this mixin. Fixed with an ownership token (`_screenHeaderOwner` in `screen_header.dart`) — `dispose()` only clears the header if this screen is still the current owner (nothing else has claimed it since); see `design_system_guide.md` §5.1 for the full trace of both scenarios this resolves. Any future change to this mixin must preserve the ownership check, not just the `RouteObserver` hook, or one of the two bugs comes back.

### Line-items grid — `SakalTableHeaderBar` (desktop) + `SakalLineItemCard` (mobile), never the card on desktop
Every screen with a product/item line list branches on `isMobile`: mobile → `SakalLineItemCard` per line (dark-headed card, unchanged shape); desktop → one `SakalTableHeaderBar` shown once above the list + a plain bordered `Row` per line (bottom border, no card chrome), with cell widths/flex matching the header exactly so columns stay pixel-aligned. **Never render `SakalLineItemCard` unconditionally** — confirmed live 2026-08-06 on Sales Quotation Entry and GRN Entry, both showing tall repeating mobile-style cards on desktop, wasting significant vertical space compared to a compact table. A column whose visibility varies per-row (e.g. a "Converted" or "Barcode" column that only some lines populate) still gets a header cell if ANY row could show it — compute that once (`_lines.any(...)`, or a company-level setting rather than a per-row one), and render a blank placeholder cell (not a shifted layout) on rows where it doesn't apply, so columns never drift out of alignment row-to-row.
**Mobile enhancement**: within each `SakalLineItemCard`, pair secondary fields into a 2-column grid via `SakalFieldRow(isMobile: true, children: [a, b])` (chunk the field list two at a time) instead of leaving them as a loose `Wrap` — predictable pairing instead of however-it-happens-to-wrap. `SakalLineItemCard.fields` is required but accepts `const []`; put the grid in its `body` slot instead.
**Mandatory self-check**: grep the screen file for `SakalLineItemCard(` — every call site must be inside an `isMobile ? ... : ...` branch (or an `if (isMobile) { return ...; }` early-return followed by the desktop path) with a `SakalTableHeaderBar`-based desktop counterpart, never called unconditionally.
Template: `sales_invoice_entry_screen.dart:2107-2268` (the original correct implementation), `sales_quotation_entry_screen.dart`/`grn_entry_screen.dart` (2026-08-06 ports, including the mobile 2-column grid and the per-row-optional-column handling above).
...
**As of 2026-08-19: fully complete app-wide.** Every remaining module's `_buildPrintDocument()` now supplies `'signatures'` (top-level, sibling of `'header'`, never nested inside it — that nesting was the actual root cause of every "blank Prepared By" report, not a data-population gap as first suspected each time). 8 modules (GRN, Purchase Return, Purchase Invoice, Material Issue, Stock Transfer Request/Transfer/Receipt, Stock Adjustment) had no `_users`/user-lookup plumbing at all yet — added `getUsers()`/`getUsersForAutocomplete()` through the remote datasource + repository interface + impl for each, not just the screen. Stock Count is a deliberate exception: it has no approve step of its own (only Submit — approval happens one level up on Stock Count Review), so its `authorised_by` binds to its own `submitted_by` column instead of a nonexistent `approved_by`. See the `PrintElementType.block` entry below for the same-session letterhead redesign this shipped alongside.

### `PrintElementType.block` — a guaranteed-aligned multi-line group within one row (2026-08-19)
The flat x/y/w row model (see the class comment on `PrintElement` in `print_models.dart`) has **no way to make one row's column boundary provably match another's** — three separate attempts at keeping a multi-line company-info/title block "aligned" via matching flex ratios across sibling rows (identical `w:` values, then a fixed-width shared column) all still drifted out of alignment in the real renderer, confirmed live, non-negotiably, by the user pointing at the rendered PDF each time despite the flex math appearing correct on paper. The only actual fix: a new `block` element type — a fixed vertical stack of sub-elements (each a normal `field`/`text` element with its own bind/label/font) rendered as **one `pw.Column`**, so line 2 sits directly under line 1 *by construction*, not by any cross-row matching at all.
- Every letterhead across all 22 templates (Journal Voucher + the shared Payment/Receipt/Contra `voucher_default_template.dart` + all 20 transaction templates) is now ONE row: `logo` (x:1) + `company_block` (x:2, lines: company_name/address/city) + `title_block` (x:3, lines: title/doc-no/date, `font: PrintFont(align: PrintAlign.right)` on the block itself, right-aligned) — replacing the old pattern of a big centered ALL-CAPS banner title below a divider, with separate doc-no/date fields further down.
- **A right-aligned block needs `pw.Row(mainAxisAlignment: end)` wrapping the Column, NOT `pw.SizedBox(width: double.infinity)`.** The SizedBox approach was tried first and looked like it worked — but only because "Journal Voucher" itself is wide enough to nearly reach the true margin anyway; shorter titles on other documents (e.g. "Purchase Return") exposed that the Column was still shrink-wrapping to its own widest line, leaving a visible gap short of the same right margin the divider/table below it correctly reaches. `pw.Row`'s own space-filling behavior is far more basic and reliably implemented in this `pdf` package than an infinite-width `SizedBox` nested inside an `Expanded`.
- The logo's own displayed size in any letterhead is capped to a compact 0.5in box (`pdf_canvas_renderer.dart`'s image case) regardless of the company's configured `logo_width_inch`/`logo_height_inch` — a full 1in-default logo shared a row with 2-3 lines of text either way (beside it or on its own row), and made that row far taller than the text needed, wasting significant vertical space. The company's full configured size still applies elsewhere (the separate Reports PDF header).
- Model/renderer support (`print_models.dart`'s `PrintElement.lines` + `fromJson`/`toJson`, both `pdf_canvas_renderer.dart` and `pdf_flow_renderer.dart`'s `block` case, `print_template_designer_screen.dart`'s `_ElementDraft.lines` + all 4 exhaustive `PrintElementType` switches) is generic and reusable for any future multi-line-group need, not letterhead-specific. The designer's own "change element type" dropdown deliberately excludes `block` as a selectable target for an existing non-block element — there's no per-line editor UI yet (Phase 2), so it only ever appears where a default template already places one; opening such a template in the designer still preserves its `lines` on save even without a rich editor for them.
- Accent styling that shipped alongside in the same pass, applied to every template: `div1`(under the letterhead)/`div3`(before signatures) upgraded from the default thin grey `line` to a thicker navy one via `h: 1.4, font: PrintFont(colorHex: '#1B3A6B')` — a `line` element's `h`/`font.colorHex` are opt-in thickness/color overrides only when set away from the type's own defaults (`h: 10`, `colorHex: '#000000'`), so every template that doesn't explicitly set them renders identically to before; a `div_totals` thin rule (`h: 0.5`) added directly above every totals block that has one; "sign above the line" thin rules added above every signature field, matching however many signature columns that specific document actually has (one/two/three) and their real widths, never assumed uniform. Shared print footer (`Printed By: <user> | Printed On: <timestamp>` left-aligned, `Page N of M` right-aligned, `pw.Row(mainAxisAlignment: spaceBetween)`) now wired via `printedByName`/`printedOn` params on every entry screen's `PrintEngine.printDocument()` call, not just Finance vouchers.
...
### The three root tenancy tables are RLS-critical too, not just leaf tables
`ric_clients`/`ric_companies`/`ric_locations` (migration 001) shipped with a permissive `dev_allow_all_*` policy (`USING (true) WITH CHECK (true)`) — its own comment said "tighten before production deployment," but no later migration ever did, since every subsequent table followed the `auth_rw_<table>` convention from creation and these three predate that convention entirely. Found live 2026-09-14 (Production-Readiness Roadmap, Phase B prep): any tenant's JWT could `SELECT` all rows from all three tables with no filter, leaking every OTHER tenant's client/company/location names. Fixed in migration 188. Scoping is NOT the standard `auth_rw_<table>` shape since these tables sit ABOVE `client_id`/`company_id` rather than carrying them as ordinary columns:
- `ric_clients` has no `client_id`/`company_id` column — it IS the client. Policy: its own `id` = JWT's `client_id` claim.
- `ric_companies` has `client_id` but no `company_id` — it IS the company. Policy: `client_id` matches AND its own `id` = JWT's `company_id` claim.
- `ric_locations` already carries both — standard `auth_rw_<table>` pattern applies directly.
**Lesson: a table predating the RLS convention is not "grandfathered in safe" — it's exactly the kind of table a later audit needs to re-check, since nothing ever forces old tables to pick up a new convention automatically.** `fn_register_client`/`fn_login` are unaffected (SECURITY DEFINER bypasses RLS/GRANTs on the calling role entirely).
...
### Server-side approve/reverse permission check (migrations 108/109) — `fn_check_approve_permission`
A real security gap found in a 2026-07-27 QA-checklist pass: the Flutter UI gated Approve via `ScreenPermissionMixin`'s `canApprove` (from the cached menu-permissions list), but no backend function ever re-checked it — `fn_post_finance_voucher` (Journal/Contra/Payment-Receipt Voucher's shared approve path), `fn_approve_expense_voucher`, and `fn_reverse_voucher` only ever checked document status + period/backdate.
...
- **No JWT context at all → skip, not deny.** pgTAP tests (and migrations) call these functions directly via SQL with no `request.jwt.claims` ever set — same trust boundary RLS itself already assumes for a privileged/direct caller. `fn_check_approve_permission` treats a NULL/unparseable JWT as "nothing to enforce" rather than raising, so every pre-existing 105/106/107 pgTAP assertion keeps passing unchanged; `10x_..._test.sql` files explicitly `set_config('request.jwt.claims', ...)` to exercise the real enforcement path instead.
- **`fn_post_finance_voucher` maps `voucher_type_code` → `feature_code`**: `'JV'` → `FN-JRN`, `'CTR'` → `FN-CTR`, `'CRV'/'BRV'/'CPV'/'BPV'` → `FN-PRV` (Payment/Receipt Voucher, `finance_voucher_entry_screen.dart`). `fn_reverse_voucher` maps the same, plus `'EXP'` → `FN-EXP` (the posted GL voucher's own `voucher_type_code` — the posting code, never `'EXV'` the document-numbering code, per Expense Voucher's numbering-vs-posting split).
- **Migration 108 originally shipped WITHOUT the `FN-PRV` mapping** — grepping the whole backend (`fn_seed_client_modules.sql` and every migration) found no seeded `feature_code`/`screen_name` row for `/finance/voucher-list` anywhere, so it looked like a pre-existing, unrelated menu-wiring gap and was deliberately left ungated rather than guessed at. **The row turned out to exist** — created manually, directly in Supabase, outside of any migration (`FN-PRV` / `Payment/Receipt Voucher` / `/finance/voucher-list`), which is exactly why grepping migrations never found it. Migration 109 closed the loop once this was confirmed: backfills `FN-PRV` for any other existing company via the same `INSERT ... SELECT FROM ric_companies JOIN ric_system_modules ... ON CONFLICT DO UPDATE` shape as migration 107's own `FN-EXP` backfill (never overwrites an existing company's own `feature_name`/`screen_name`/`serial_no`), adds `FN-PRV` to `fn_seed_client_modules.sql` itself so new clients get it automatically, and extends `fn_post_finance_voucher`/`fn_reverse_voucher`'s mapping. **Lesson: grepping migrations/functions proves what's SEEDED, not what's LIVE — a live database can carry manual patches no file on disk reflects. When a gap looks surprising ("this screen would be totally unreachable"), it's worth confirming against the live DB before treating the grep result as ground truth.**
...
- **Rollout to every other module, batched (Purchase → Inventory → Sales), same pattern, same helper — no new infra needed.** 20 more `fn_approve_*`/`fn_cancel_*` functions across the app had the identical gap (client-side `canApprove` only, no server-side re-check). **Migration 110 (Purchase) covers all 4**: `fn_approve_purchase_order` → `PR-PO`, `fn_approve_grn` → `PR-GRN`, `fn_approve_purchase_invoice` → `PR-INV`, `fn_approve_purchase_return` → `PR-RET` (6-param signature, extra `p_reopen_po BOOLEAN` before `p_approved_by` — preserved). Same insertion rule as 108/109: `PERFORM fn_check_approve_permission(...)` goes immediately after each function's existing `IF v_header.status != 'DRAFT' THEN ... END IF;` block, before period/backdate checks — every other line of each function reproduced verbatim, never truncated (see [[feedback_check_latest_function_signature]]).
- **Migration 112 (Inventory) covers all 8**: `fn_approve_material_requisition` → `IN-MRQ`, `fn_approve_material_issue` → `IN-MIS`, `fn_approve_stock_transfer_request` → `IN-STR`, `fn_approve_stock_transfer` → `IN-TRF`, `fn_approve_stock_receipt` → `IN-SRC`, `fn_approve_stock_adjustment` → `IN-ADJ` (guarded, see below), `fn_approve_opening_stock` → `IN-OPN`, `fn_approve_stock_count_review` → `IN-CNR` (`RETURNS TEXT`, not `VOID` — preserved). **`fn_approve_stock_adjustment`'s `IN-ADJ` check needed the SAME guard as migration 111's fix, applied proactively this time instead of discovered as a live regression**: `fn_approve_stock_count_review` doesn't go through a generic posting engine — it directly composes `fn_save_stock_adjustment` + `fn_approve_stock_adjustment` (tagging the adjustment's header `source_doc_type='STOCK_COUNT_REVIEW'`, a nullable column added in migration 079 specifically for this traceability). Checking `IN-ADJ` unconditionally would have silently required a Stock Count Review approver to ALSO hold separate Stock Adjustment permission — the identical bug class 111 fixed for GRN/Purchase Return's auto-posted JV. Fixed with the same recipe: `IN-ADJ` is only checked `WHEN v_header.source_doc_type IS NULL` (a direct human approval on the Stock Adjustment screen); an auto-posted-from-Review adjustment is already gated by the Review's own `IN-CNR` check one level up. Verified by a dedicated pgTAP pair in `112_inventory_approve_permission_test.sql`: a user with `IN-CNR` but explicitly NOT `IN-ADJ` can approve a Review (composition bypasses the check) but is still correctly denied approving a DIRECT Stock Adjustment (the guard's scope, not a blanket skip). **Any future module that composes another module's approve function directly (not through a shared posting engine) needs this same `source_doc_type IS NULL`-style guard question asked before shipping — this is now the second occurrence of the identical bug shape, once found live, once caught proactively.**
- **Real regression caught by migration 110's own pgTAP suite, fixed in migration 111**: `fn_post_finance_voucher`'s `IF v_header.voucher_type_code = 'JV' THEN PERFORM fn_check_approve_permission(..., 'FN-JRN')` (108) fires unconditionally — but GRN's and Purchase Return's own accrual/reversal postings ALSO go through this same shared function (`fn_approve_grn`/`fn_approve_purchase_return` call `fn_post_voucher(..., 'JV', ...)`, which composes `fn_save_finance_voucher` + `fn_post_finance_voucher`). Since 108 shipped, approving a GRN or an unbilled Purchase Return silently ALSO required the acting user to hold unrelated Journal Voucher (`FN-JRN`) permission — an internal implementation detail (which `voucher_type_code` the accrual happens to post under) leaking into a different module's permission model. **Fix (111)**: `fn_post_voucher` already stamps `source_doc_type` (`'GRN'`, `'PURCHASE_RETURN'`, ...) onto the header *before* calling `fn_post_finance_voucher`, so a direct human JV/CTR entry (`source_doc_type IS NULL` — `fn_save_finance_voucher` is called directly there with no `source_doc_type` key) is reliably distinguishable from an auto-posted one. The permission check is now gated `AND v_header.source_doc_type IS NULL` on both the `JV` and `CTR` branches — only a direct voucher entry re-checks `FN-JRN`/`FN-CTR`; an auto-posted voucher already got checked at its OWN calling module's level (`PR-GRN`, `PR-RET`, ...), so no coverage is lost. **Any future module that auto-posts through `fn_post_voucher` under `voucher_type_code IN ('JV','CTR')` is automatically covered by this same guard — no per-module change needed.**
- **Migration 111 itself introduced a SECOND, more serious regression, fixed in migration 113**: 111 reproduced `fn_post_finance_voucher`'s body from **migration 108's own definition** rather than the true latest — migration 109 had already extended the same function with a third branch (`voucher_type_code IN ('CRV','BRV','CPV','BPV') THEN fn_check_approve_permission(..., 'FN-PRV')`). Building 111 from the stale (108) body silently **dropped that branch entirely** — from the moment 111 shipped until 113 fixed it, posting/approving ANY Payment or Receipt Voucher performed NO server-side permission check at all, the exact gap 109 was built to close, silently reopened. Root cause: 111 was built reactively, under time pressure, and skipped the "grep every migration for the function's current definition" discipline used everywhere else (see [[feedback_check_latest_function_signature]]) — reading "the complete current body" from *a* migration is not the same as reading it from *the latest* one. **Migration 113 restores the branch and, since already touching every branch, proactively extends the same `source_doc_type IS NULL` guard to it** — needed because migration 114 (Sales) has Sales Invoice compose a `CRV` settlement voucher directly via `fn_save_finance_voucher`/`fn_post_finance_voucher` (never `fn_post_voucher`) for immediate cash collection; without the guard, approving a cash Sales Invoice would have silently also demanded unrelated `FN-PRV` permission. Migration 114 tags that composed voucher's header `source_doc_type='SALES_INVOICE'` (mirroring `fn_post_voucher`'s own convention, and adding traceability that voucher never had) so the guard exempts it correctly while still gating any voucher entered directly via the Payment/Receipt Voucher screen. **Lesson, generalizing further: when fixing a shared function reactively (a live-test failure, not a planned rollout step), the "grep for the latest definition" step is not optional even under time pressure — it is exactly the moment skipping it is most likely, and most costly.**
- **Migration 114 (Sales, final batch) covers all 8**: `fn_approve_sales_quotation` → `SL-QUO`, `fn_approve_sales_order`/`fn_cancel_sales_order` → `SL-SO`, `fn_approve_sales_invoice`/`fn_cancel_sales_invoice` → `SL-INV`, `fn_approve_sales_return` → `SL-RET`, `fn_approve_sales_delivery` → `SL-DEL`, `fn_approve_cash_receipt` → `SL-RCP`. **A THIRD occurrence of the composition-guard pattern, found here proactively (not after a failed test)**: `fn_approve_sales_invoice`, `fn_approve_sales_return`, and `fn_approve_cash_receipt` each compose a CRV/CPV settlement voucher directly via `fn_save_finance_voucher`/`fn_post_finance_voucher` (never `fn_post_voucher`) whenever cash actually moves — immediate collection on a cash sale, a cash refund on a return, and bill settlement respectively — and none of these composed vouchers were ever tagged with `source_doc_type`. Without the guard, approving any of these would have silently also required unrelated `FN-PRV` permission — six call sites total (each of the three functions has a LOCAL-currency leg and a BASE-currency leg). Fixed by tagging each composed voucher's header (`UPDATE rih_finance_headers SET source_doc_type = 'SALES_INVOICE'/'SALES_RETURN'/'CASH_RECEIPT', source_doc_no = ..., source_doc_date = ...`) immediately after `fn_save_finance_voucher` and before `fn_post_finance_voucher`, mirroring `fn_post_voucher`'s own convention exactly and adding traceability these vouchers never had. Verified in `114_sales_approve_permission_test.sql`: `v_user_ok` holds every `SL-*` feature_code used but is deliberately never granted `FN-PRV`, and still successfully approves a CASH invoice, a cash-refund return, and a bill-settling receipt — each assertion checking not just "no error" but that the composed voucher's own number actually got recorded on the header (proof the composition genuinely completed, not merely skipped a check and stalled).
