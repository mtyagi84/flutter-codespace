...
| Inventory (stock, transfers, adjustments) | Material Requisition/Issue, Stock Transfer (Request/Transfer/Receipt), and Stock Adjustment all done. Barcode traceability + company-config field gating (Pack/Loose Qty, Barcode) audited complete app-wide. Reports: Stock Balance by Location (location-scoped), Stock Value by Location (weighted-avg cost across locations, category subtotals, dual-currency columns), and Stock Details (Opening/Inward/Outward/Closing per product over a date range, reads `ril_stock_ledger` directly) — all three gated behind the engine-wide `auto_load`/"Run Report" mechanism (migration 148). Stock Details' Product filter is also the first use of a new generic cascading-lookup-filter engine capability (migration 149, `ric_report_filters.depends_on_*`) — reusable for future Sales/Inventory report filters |
