-- ============================================================
-- 016_add_allowed_column.sql
-- Adds add_allowed to ric_user_menus and rebuilds all three
-- permission functions to carry the new column.
--
-- WHY: edit_allowed previously conflated "create new" with
-- "modify existing". Separated so you can grant a user the
-- right to create invoices without letting them edit posted ones.
-- Run after 012_permissions_fn.sql
-- ============================================================


-- ── Step 1: Add column ────────────────────────────────────────
ALTER TABLE ric_user_menus
    ADD COLUMN add_allowed boolean NOT NULL DEFAULT false;
...
AS $$
    SELECT
        ...
        COALESCE(um.add_allowed,          false)    AS add_allowed,
...
-- ── Step 3: fn_upsert_user_permission ────────────────────────
CREATE OR REPLACE FUNCTION fn_upsert_user_permission(
    ...
    p_add_allowed           boolean DEFAULT false,
...
BEGIN
    INSERT INTO ric_user_menus (
        ...
        view_allowed, add_allowed, edit_allowed,
        ...
        p_view_allowed, p_add_allowed, p_edit_allowed,
        p_approve_allowed, p_copy_allowed, p_excel_upload_allowed,
...
-- ── Step 4: fn_copy_user_permissions ─────────────────────────
