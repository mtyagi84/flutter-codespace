-- ============================================================
-- Migration 039: fn_save_purchase_order concurrency fix
-- ============================================================
-- 031 already ran in Supabase, so this ships as its own follow-up
-- CREATE OR REPLACE rather than editing 031 in place (an edit to an
-- already-run migration file never reaches deployed Supabase schema).
--
-- Gap: the edit-path status check used a plain SELECT, not SELECT ... FOR
-- UPDATE, before checking status != 'DRAFT'. That leaves a window where a
-- concurrent fn_approve_purchase_order could commit between this function's
-- status check and its DELETE+INSERT of lines/charges. The same fix was
-- applied to fn_save_grn in migration 038 for the identical reason — see
-- that file's comment on fn_save_grn for the full explanation.
--
-- Everything else in this function is byte-for-byte identical to 031's
-- version — only the edit-path status check changed.
-- ============================================================
