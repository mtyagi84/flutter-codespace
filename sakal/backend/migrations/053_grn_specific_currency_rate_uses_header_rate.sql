-- ============================================================
-- Migration 053: fn_approve_grn — cost_price_specific's rate must reuse
-- the header's own rate_to_base/rate_to_local too, same as party_rate (052)
-- ============================================================
-- Same class of bug as 052, on the product-specific-currency side instead
-- of the account side: rim_products.cost_currency_id lets an item's
-- moving-average cost be tracked in its own currency (e.g. an EUR-costed
-- product bought on a USD-base, CDF-local company). fn_approve_grn always
-- called fn_get_exchange_rate() for v_rate_to_specific whenever the
-- product had a cost currency set — even when that currency was exactly
-- the GRN's own currency, the base currency, or the local currency, all of
-- which already have a known, user-confirmed rate sitting right there on
-- the header. That's a fresh, possibly-different system rate silently
-- overriding the rate the user just typed in for this exact document.
--
-- Fix: same three-way shortcut as 052's party_rate, applied to
-- v_rate_to_specific, checked before falling back to fn_get_exchange_rate:
--   product currency = GRN currency   -> rate 1                (new)
--   product currency = base currency  -> v_header.rate_to_base  (new)
--   product currency = local currency -> v_header.rate_to_local (new)
--   product currency IS NULL          -> v_rate_to_base         (unchanged;
--     no specific-currency tracking configured, specific = base)
--   anything else                     -> fn_get_exchange_rate   (unchanged;
--     genuine third currency, no user-entered rate to reuse)
--
-- New migration, not an edit to 052 — 052 may already be deployed.
-- ============================================================
