-- ============================================================
-- Migration 057: fn_approve_grn — v_rate_to_base must reuse the header's
-- own confirmed rate_to_base too, same class of bug as 052/053
-- ============================================================
-- Found while writing pgTAP tests for Purchase Bill (054/055): approving a
-- foreign-currency GRN failed outright with "No exchange rate found for
-- <GRN_CCY> -> <BASE_CCY>" even though the header already carries a
-- user-confirmed rate_to_base — the exact same GRN-currency-to-base-currency
-- conversion fn_get_exchange_rate was being asked to re-derive from scratch.
--
-- 051/052/053 already fixed this exact class of bug for party_rate and
-- cost_price_specific (never re-fetch a fresh system rate when the header
-- already has a user-confirmed one for that currency pair) but v_rate_to_base
-- itself — the one that feeds v_unit_cost_base, i.e. rim_product_location.
-- cost_price and ril_cost_price_history, i.e. actual inventory valuation —
-- was left doing a fresh fn_get_exchange_rate lookup unconditionally. Beyond
-- the hard failure when no rate row exists at all, even when one does exist
-- it can silently differ from the header's own rate_to_base, so the stock
-- valuation and the journal voucher's Stock Dr amount (which already uses
-- v_header.rate_to_base, see the 'base_amount' lines in this same function)
-- could end up computed at two different rates for the exact same document.
--
-- Fix: v_rate_to_base := v_header.rate_to_base — no lookup needed at all,
-- since that field IS this exact conversion, already confirmed by the user
-- when they entered/edited the GRN's header rate.
--
-- New migration, not an edit to 053 — 053 may already be deployed.
-- ============================================================
