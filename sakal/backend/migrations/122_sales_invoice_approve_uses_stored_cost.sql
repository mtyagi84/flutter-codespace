-- ============================================================
-- Migration 122: fn_approve_sales_invoice — use the stored cost_price
-- ============================================================
-- User-specified: the cost locked in at save time (rid_sales_invoice_lines
-- .cost_price, resolved by migration 121's fn_save_sales_invoice per
-- rules 1-4) is what actually posts to the COS voucher — replacing the
-- old live re-lookup of rim_product_location.cost_price. Single source
-- of truth between what was validated/shown at save and what actually
-- posts; if the item's moving-average cost has since moved (e.g. a later
-- GRN), the invoice still uses the cost that was locked in when it was
-- created, not a fresher number.
--
-- cost_price is always stored in the INVOICE's own currency (rule 1,
-- regardless of which of rules 2/3/4 computed it) — recovering the
-- base-currency figure needed for the COS voucher (always base currency)
-- uses v_header.rate_to_base, the SAME rate every other amount in this
-- function already converts through (see the Customer DR / Sales CR
-- lines above) — not a fresh fn_get_exchange_rate lookup, which would
-- introduce a second, inconsistent rate source for the same invoice.
--
-- Also adds a safety-net check: cost_price could still be NULL here if
-- this DRAFT was saved OFFLINE (fn_save_sales_invoice's
-- p_enforce_cost_check=false path, migration 121) — Approve is always
-- online, so this is where that deferred validation actually happens for
-- real, same "server-side re-check, never fully trust an offline-
-- originated draft" posture as every other approve-time validation in
-- this schema.
