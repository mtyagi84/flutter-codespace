-- ============================================================
-- Migration 123: Sales Return — cost_price carried from the invoice line
-- ============================================================
-- User-specified rule 6: rid_sales_return_lines.cost_price (added in
-- migration 120) is populated verbatim from the matching
-- rid_sales_invoice_lines row (matched by invoice_no/invoice_date/
-- serial_no = invoice_line_serial) at SAVE time — never recomputed,
-- since a return can only exist against an already-approved invoice
-- whose own cost is already locked in. Same value is then used to post
-- the return's own reversing COS voucher at Approve time, replacing the
-- old join-back-through-the-original-COS-voucher lookup — a genuine
-- simplification this new column enables, not just a new feature.
-- ============================================================

-- ------------------------------------------------------------
-- fn_save_sales_return — copy cost_price from the invoice line
-- ------------------------------------------------------------
...
DECLARE
    ...
    v_orig_cost_price NUMERIC;   -- NEW (123)
BEGIN
    ...
    LOOP
        -- NEW (123): cost_price copied verbatim from the matching invoice
        -- line — never recomputed. Rule 6.
        SELECT cost_price INTO v_orig_cost_price
        FROM rid_sales_invoice_lines
        WHERE client_id = v_client_id AND company_id = v_company_id
          AND invoice_no = v_invoice_no AND invoice_date = v_invoice_date
          AND serial_no = (v_line->>'invoice_line_serial')::integer;
...
-- ------------------------------------------------------------
-- fn_approve_sales_return — read cost_price directly, no more join
-- ------------------------------------------------------------
...
BEGIN
    ...
    --    above) — never a fresh current-average lookup, to keep this
    --    reversal symmetric with what the invoice itself posted. This
    --    replaces the old join back through rid_finance_lines/
    --    rih_finance_headers to the invoice's own posted COS voucher —
    --    same result, but now a plain column read since the invoice line
    --    itself carries its own cost directly.
    IF v_invoice.stock_dispatch_mode = 'IMMEDIATE' THEN
        ...
        LOOP
            ...
            -- NEW (123): cost_price copied from the invoice line at save
            -- time — a NULL here means either the invoice predates
            -- migration 121 (no cost_price ever populated for it) or the
            -- copy at save time genuinely found nothing, which shouldn't
            -- happen for an APPROVED invoice (fn_approve_sales_invoice,
            -- migration 122, hard-blocks a NULL cost_price at approve
            -- time going forward) — surfaced as a clear error either way,
            -- rather than silently posting a zero-cost reversal.
