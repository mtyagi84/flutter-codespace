-- ============================================================
-- 177_grn_landed_cost_price_fix.sql
--
-- Real bug found live by the user testing a GRN with a per-line discount
-- and an apportioned Transport charge: ril_cost_price_history.cost_price_in
-- / cost_price_in_specific (and therefore rim_product_location.cost_price)
-- were populated from v_line.rate — the raw supplier price BEFORE discount
-- and BEFORE the line's own apportioned share of additional charges.
--
-- Meanwhile the GL Stock Dr posting a few lines further down in the same
-- function already computes the CORRECT landed figure:
--   v_taxable_amount := v_line.final_amount - v_line.tax_amount;   -- net of VAT, net of discount
--   v_trans_amt       := v_taxable_amount + v_line.charge_amount;   -- + apportioned charge
-- So the stock valuation and the GL Stock account balance disagreed for any
-- GRN with a discount and/or a charge — current_stock * cost_price no
-- longer equalled the GL Stock account balance for that product.
--
-- Fix: compute v_taxable_amount BEFORE the unit-cost lines (moved up from
-- its later use in the GL section) and derive landed unit cost as
--   (v_taxable_amount + v_line.charge_amount) / v_line.base_qty
-- in the GRN's own currency, then convert to base/specific exactly as
-- before. VAT/tax is deliberately excluded (v_taxable_amount already nets
-- it out) — tax is a recoverable asset, not part of stock cost, matching
-- the VAT-deferral design established in migration 050.
--
-- Verified against the user's own worked example: 10 EUR list price, 5%
-- discount => 9.5, plus this line's apportioned share of a 10 EUR Transport
-- charge (~0.0606/unit here) => landed unit cost ~9.5606 EUR, converted to
-- base currency — exactly what this fix now computes instead of the
-- previous 10 EUR flat.
--
-- Verbatim reproduction of fn_approve_grn's current live body (from
-- migration 110), same signature, same every other line — only the unit-
-- cost computation changes, per the "grep every migration for the current
-- definition, never trust the original file" rule.
-- ============================================================
...
BEGIN
    ...
    LOOP
        ...
        -- FIX (177): landed unit cost = net-of-discount, net-of-VAT item
        -- value PLUS this line's own apportioned charge share, divided by
        -- qty — NEVER v_line.rate directly, which is the gross per-unit
        -- price before discount and before the apportioned charge. Moved
        -- v_taxable_amount's computation up from its later use in the GL
        -- Stock Dr section below so stock valuation and that same GL line
        -- always agree on what one unit landed at. VAT is deliberately
        -- excluded — it's a recoverable asset, not part of stock cost.
        v_taxable_amount := v_line.final_amount - v_line.tax_amount;
        IF v_line.base_qty > 0 THEN
            v_unit_landed := (v_taxable_amount + v_line.charge_amount) / v_line.base_qty;
        ELSE
            v_unit_landed := 0;
        END IF;
        v_unit_cost_base     := v_unit_landed * v_rate_to_base;
        v_unit_cost_specific := v_unit_landed * v_rate_to_specific;
        ...
        -- GRN's OWN transaction currency. v_taxable_amount already computed
        -- above for the landed-cost calculation — reused here unchanged.
