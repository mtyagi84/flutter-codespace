-- ============================================================
-- Migration 189: Zambia COA gains 3 missing standard lines --
-- Office Equipment, Accumulated Depreciation, Depreciation Expense
-- ============================================================
-- Found 2026-09-18 while onboarding the first real tenant (Shanju
-- Investment Limited, a Zambia steel/hardware trader): its real Chart of
-- Accounts needs a fixed-asset "Office Equipment" line, a contra-asset
-- "Accumulated Depreciation" line, and a "Depreciation Expense" line --
-- none of which exist in the ZAMBIA branch of fn_seed_chart_of_accounts
-- (migration 180). OHADA has its own "Accumulated Depreciation" (2800)
-- and "Depreciation Expense" (6800) equivalents; INDIAN has neither.
-- Confirmed genuinely missing, not a naming mismatch, by reading the full
-- current ZAMBIA branch directly (migration 180 is still the latest
-- definition -- grepped every migration for a later CREATE OR REPLACE,
-- found none).
--
-- User's explicit instruction: add these as permanent, standard seed
-- lines (not a one-off custom account for this tenant alone) so every
-- future Zambia tenant gets them too -- this is additive only, touching
-- ONLY the ZAMBIA branch; OHADA and INDIAN are reproduced byte-for-byte
-- unchanged from migration 180, per this project's own "grep the latest
-- definition, reproduce the whole function, never truncate" rule for any
-- change to a function this central.
--
-- New codes (siblings of the existing ZAMBIA leaves, same numbering
-- convention):
--   1245 Office Equipment          -- posting leaf, under 1200 Non Current Assets, sibling of 1240/1250/1260
--   1295 Accumulated Depreciation  -- posting leaf, under 1200 Non Current Assets (one generic contra-asset
--                                     line, matching OHADA's own single 2800 line -- a tenant needing separate
--                                     per-asset-category depreciation tracking can still add its own child
--                                     accounts on top of this via the ordinary Chart of Accounts screen)
--   5260 Depreciation Expense      -- posting leaf, under 5200 Operating Expense, sibling of 5210-5250
--
-- Existing Zambia tenants (there IS already one at this point -- Shanju's
-- own COA was seeded via 180 BEFORE this migration existed) do NOT get
-- these 3 lines automatically: fn_seed_chart_of_accounts only ever runs
-- ONCE per company, at accounting-setup time, gated by is_coa_seeded.
-- Any tenant already past that point needs these 3 lines added manually
-- via the ordinary Chart of Accounts screen -- a few rows, not a bulk
-- operation, and exactly what that screen exists for.
-- ============================================================
...
BEGIN
    ...
    --
    -- Migration 189 additions (2026-09-18): 1245 Office Equipment,
    -- 1295 Accumulated Depreciation, 5260 Depreciation Expense -- see
    -- this migration's own header comment for why.
    ELSIF p_std = 'ZAMBIA' THEN
        ...
        ('1245', 'Office Equipment',              '1200', TRUE,  'General'),
        ('1250', 'Motor Vehicles',                '1200', TRUE,  'General'),
        ('1260', 'Computer & IT Equipment',       '1200', TRUE,  'General'),
        ('1270', 'Intangible Assets',             '1200', TRUE,  'General'),
        ('1280', 'Capital Work In Progress',      '1200', TRUE,  'General'),
        ('1290', 'Investments',                   '1200', TRUE,  'General'),
        ('1295', 'Accumulated Depreciation',      '1200', TRUE,  'General'),
        -- Liabilities
        ('2000', 'Liabilities',                   NULL,   FALSE, 'General'),
        ('2100', 'Current Liabilities',           '2000', FALSE, 'General'),
        ('2110', 'Trade Payables',                '2100', FALSE, 'Supplier'),
        ...
        ('5240', 'HR Expenses',                   '5200', TRUE,  'General'),
        ('5250', 'Maintenance Expenses',          '5200', TRUE,  'General'),
        ('5260', 'Depreciation Expense',          '5200', TRUE,  'General'),
        ('5300', 'Finance Cost',                  '5000', FALSE, 'General'),
        ('5310', 'Bank Charges',                  '5300', TRUE,  'General'),
        ('5320', 'Interest On Loan',              '5300', TRUE,  'General'),
        ('5330', 'Forex Loss',                    '5300', TRUE,  'General'),
