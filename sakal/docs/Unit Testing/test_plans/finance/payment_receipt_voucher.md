...
| **PRV-C05** | **[OPEN BUG, found 2026-09-13] Party Amount doesn't populate on an On Account line** | Add an account line, mark "On Account", type a value into Amount | **Expected**: Party Amount field auto-populates immediately (converted via the account's own currency, per CCC #3). **Actual**: Party Amount stays blank/unchanged. | **High** | **Failed** | Not yet fixed — likely the `onChanged` handler for the Amount field isn't wired to recompute Party Amount for the On-Account case specifically (the "Settle Against Bill" case may already do this correctly, since PRV-C02 expects it to work — verify that assumption directly, don't assume) |
| PRV-C06 | Verify PRV-C02's own settle-against-bill amount-population still works | Repeat PRV-C02, watch Party Amount as Amount is typed | Confirms whether the bug is On-Account-specific or affects the whole field, which changes where the fix needs to go | High | Not Started | Directly informs the fix for PRV-C05 |
