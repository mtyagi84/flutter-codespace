...
**Real bugs caught only once actually run against Supabase (self-review missed both — same pattern documented elsewhere in this project: some things only surface at execution time, not reading):**
6. `ALTER TABLE rim_voucher_types ADD CONSTRAINT rim_voucher_types_nature_check CHECK (...)` failed live with `23514: check constraint ... is violated by some row` — the constraint had already been widened TWICE since the original 017 definition (migration 031 added `PURCHASE`, migration 081 added `SALES` on top of that), and this migration re-derived its new list from the stale original 017 values instead of grepping every migration first. Would have silently narrowed the live constraint back and broken every already-shipped PURCHASE/SALES-natured system voucher type (PO-LOC, GRN, PINV, SQ, SO, SI, ...). Fixed by extending from the actual current (081) list. Exactly the "check latest signature before editing" mistake this project has been bitten by before, generalized here to a CHECK constraint rather than a function signature.
