...
## Follow-up fixes, round 2 (same day)

1. **"Group Name" column** — the immediate parent group's `account_name`, added everywhere: on-screen (read-only field, first column both desktop and mobile), the Excel template (first exported column), and `getLines()`'s embed gained a nested `parent:rim_accounts!parent_id(account_name)`. Display-only — never matched against on upload, Account Code alone remains the match key (same as Account Name already was).
2. **Zero-balance upload was being silently dropped** — the original skip heuristic ("skip a row if every amount is 0 and Bill No is empty") meant a user correcting a wrongly-uploaded non-zero balance back to 0 had their correction silently ignored. Redesigned: the only signal for "untouched template placeholder" is now a blank Opening Balance Type — that's the one thing a freshly-downloaded, never-edited row always has. Once a real Dr/Cr type is present, the row uploads as a deliberate entry, zero amounts included.
3. **Save-time validations added** (deliberately not upload-time, per explicit instruction — upload always succeeds, Save is where the user finds out what's wrong, with a message naming the specific account): all three amounts (Base/Local/Party) must be entered together or all be exactly 0; a non-Customer/Supplier account may not have more than one line; a Customer/Supplier account may not have two lines sharing the same non-blank Invoice/Bill No + Date.
4. **Total Debit/Credit summary** — Base and Local only (never Party, since summing different accounts' own currencies together would be meaningless), computed across every line regardless of the search filter, shown above the line grid so the user can cross-check against their own Excel sheet's totals.

## Follow-up fix, round 3 (2026-08-14) — numeric Excel cells

**Bug**: every downloaded Excel export in the app (this screen's template, and the shared Reporting Engine export used by Sales Register/Account Ledger/every future report) wrote every cell — including amounts — as `xls.TextCellValue`. Excel correctly flagged these with its "number stored as text" warning, and SUM/formulas didn't work on them. `report_excel_export.dart` had explicitly flagged this as a deliberate, deferred limitation ("not attempted here without a way to verify the additional API surface compiles") — same constraint I faced this round (no local Flutter/Dart toolchain or pub-cache to inspect the `excel` package's source against).

**Fix**: applied the `excel` package's documented `xls.DoubleCellValue(double)` constructor (same single-positional-arg shape already proven for `xls.TextCellValue(String)` in this exact pinned version) to every numeric amount cell:
- `report_excel_export.dart` — new `_cellValue()` helper returns `DoubleCellValue` for any column with `dataType == 'NUMBER'` (main rows, subtotal aggregate cells, totals row), `TextCellValue` otherwise. A NUMBER column that also carries a per-row currency code (`currencyCodeColumn`) drops the inline `"CODE 123.45"` text prefix in the Excel export specifically, since the currency is already its own column on every report using this — the amount cell stays a pure number. The matrix export's pivoted numeric cells (`exportMatrix`) got the same treatment.
- `opening_balance_entry_screen.dart`'s `_downloadTemplate()` — Base/Local/Party Amount cells (both the blank-row and existing-line branches) now write `xls.DoubleCellValue(double.tryParse(...) ?? 0)` instead of `TextCellValue`.

**Not independently verified against the `excel` package's real compiled API** (no local toolchain) — relies on the package's well-documented public `CellValue` subtype constructors. `_uploadExcel()`'s read-back path (`cell?.value?.toString()`) is unchanged and should keep working since every `CellValue` subtype is expected to stringify to its raw value, but this is the one part of the fix that needs a real `flutter analyze`/`flutter test` + a manual download→edit→re-upload round trip to confirm.

## Follow-up fix, round 4 (2026-08-14) — load error + investigation of missing Group

**Bug**: opening the screen showed "Unable to load opening balance lines. Please try again." — the generic `ErrorPresenter.format` fallback, meaning either a DioException with no parseable PostgREST error body, or (more likely, since `DioClient`'s interceptor already promotes `details` into `message`) a plain Dart exception thrown from `_loadLines()`'s own row-mapping code, not the HTTP layer itself.

**Root cause**: round 2's Group Name addition gave `OpeningBalanceRemoteDs.getLines()` a **two-level-deep self-referencing embed** — `rid_opening_balance_lines -> rim_accounts!account_id(... -> parent:rim_accounts!parent_id(account_name))`. Every other `parent:rim_accounts!parent_id(...)` embed in this codebase (`accountsProvider`, `master_data_modules.dart`) is a **single-level** embed directly on `/rim_accounts` — this was the only doubly-nested self-join embed anywhere in the app, and unproven against real PostgREST.

**Fix**: `getLines()`'s select dropped back to a single-level `rim_accounts!account_id(account_code,account_name,account_nature)` embed. `_loadLines()` now resolves Group Name client-side via a new `_groupNameForAccount(accountId)` helper that looks the account up in `_postableAccounts` (already loaded from `accountsProvider`, which already carries the proven single-level `parent` embed) — same data, no fragile double-nested query.

**On "Parent group is not showing" for manually-selected accounts**: `_onAccountSelected` already reads `account['parent']?['account_name']` directly off the same `_postableAccounts` list passed into `FinanceAccountPicker` — that code path was already correct and unchanged by this fix. The most likely explanation is that the specific accounts tested (Cash USD, Cash CDF, Top Grow Ltd) genuinely have no `parent_id` assigned in this company's live Chart of Accounts, rather than a code defect — worth confirming directly in Chart of Accounts (does editing one of these accounts show a Parent Group?) before assuming further code changes are needed here.

**Separately noted, not fixed**: a 1.3px `RenderFlex` overflow in `top_bar.dart`'s `_buildScreenTitle` Row (title + help icon) appeared in the same log — pre-existing, unrelated to Opening Balance, affects the shared TopBar on any screen with a title+help-icon combination at a narrow width. Flagged for a separate follow-up, not addressed in this round.

## Follow-up fix, round 5 (2026-08-16) — Group Name really was a code bug (self-join FK direction)

Round 4 had guessed "Parent group is not showing" was a live data gap (no `parent_id` set on the tested accounts) — user confirmed Chart of Accounts genuinely shows a Parent Group for these accounts, so it was a real query bug after all.

**Root cause**: `accountsProvider`'s `parent:rim_accounts!parent_id(account_name)` embed (`master_cache_providers.dart`, also duplicated in `master_data_modules.dart`'s sync cache) disambiguated the self-referencing FK by **column name**, which PostgREST resolved to the **reverse** relationship — "children of this account" — not "this account's own parent." Every account ever shown in any picker is a leaf/postable account with zero children, so the embed always came back an empty list, silently normalized to `null` by the existing to-one defensive normalization already in `accountsProvider` (`if (parentRel is List) a['parent'] = parentRel.isNotEmpty ? parentRel.first : null` — an empty list correctly becomes `null` there, but the underlying *query* was already fetching the wrong side of the relationship). Group Name has been blank everywhere (Opening Balance, Account Ledger's Account filter, every `FinanceAccountPicker`) since Group Name was first added, not just in Opening Balance.

**Fix, attempt 1 (failed live)**: disambiguate via the FK **constraint name** (`rim_accounts_parent_id_fkey` — Postgres's default auto-generated name for the unnamed `parent_id UUID REFERENCES rim_accounts(id)` in migration 013). This is PostgREST's documented, unambiguous way to pin embed direction for a reflexive relationship — but it broke `accountsProvider` outright live ("Searched for a foreign key relationship... using the hint 'rim_accounts_parent_id_fkey'... but no matches were found"), taking down every account picker app-wide, not just Group Name. Reverted immediately.

**Fix, attempt 2 (also failed live)**: the user independently confirmed the constraint name via `SELECT conname FROM pg_constraint WHERE conrelid = 'rim_accounts'::regclass AND contype = 'f' AND confrelid = 'rim_accounts'::regclass;` → `rim_accounts_parent_id_fkey`, exactly as guessed — so the name was right, but PostgREST's schema cache apparently didn't know about it. Re-applied the same fix after asking the user to run `NOTIFY pgrst, 'reload schema';`. Still failed with the identical "no matches were found" error. Reverted again.

**Fix, attempt 3 (final)**: gave up on embedding this self-join at all. `accountsProvider` (and its `master_data_modules.dart` sync-cache duplicate) now fetch `parent_id` as a **plain scalar column** on the main query, then issue a **second, small, non-embedded query** (`id,account_name` for just the distinct parent ids actually referenced) and join client-side in Dart into the exact same `account['parent']['account_name']` shape every consumer already expects — `FinanceAccountPicker`, Opening Balance, etc. all needed zero changes. Self-referencing embeds proved unreliable against this project's live PostgREST regardless of hint form (column name → wrong direction; constraint name → rejected outright even after a cache reload) — a flat lookup has no embedding ambiguity to trip over at all, at the cost of one extra small HTTP round-trip.

**Also fixed this round**:
- Excel upload leaving a stale search filter active, making a freshly-replaced worksheet look empty ("No lines match this search" reads the same as "nothing loaded").
- Excel upload was skipping any row with a blank Opening Balance Type as an "untouched placeholder" — but per explicit user correction, a genuinely zero-balance account may never have had a reason to pick Dr vs Cr, so this wrongly dropped legitimate zero-balance entries. Every row with a real account code now loads; a blank Type defaults to `Dr` (meaningless distinction for a zero value). The "N blank row(s) ignored" wording is gone from the success toast accordingly.
