...
class MaterialIssueListScreen extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<MaterialIssueListScreen> createState() => _MaterialIssueListScreenState();
}

class _MaterialIssueListScreenState extends ConsumerState<MaterialIssueListScreen>
  ...
  Future<void> _load() async {
    ...
    try {
      final results = await Future.wait([
        ref.read(materialIssueRepositoryProvider).listIssues(
          clientId: session.clientId, companyId: session.companyId, status: _filterStatus,
        ),
        ref.read(syncEngineProvider).pendingDocumentIds('MATERIAL_ISSUE'),
      ]);
      if (mounted) {
        setState(() {
      ...
      if (mounted) setState(() { _loading = false; _error = 'Could not load material issues: $e'; });
    }
  }
  ...
  Future<void> _openNew() async {
    await context.push(RouteNames.materialIssueEntry);
    if (mounted) _load();
  }
    ...
    return Column(
      ...
      children: [
        ...
        Padding(
          ...
          child: Row(children: [
            const Expanded(child: Text('Material Issue', style: TextStyle(fontSize: 20, fontWeight: FontWeight.w700, color: AppColors.primary))),
            if (canAdd)
              FilledButton.icon(icon: const Icon(Icons.add, size: 16), label: const Text('New Material Issue'), onPressed: _openNew),
          ]),
        ),
            ...
            SizedBox(
              ...
              child: TextField(
                ...
                decoration: InputDecoration(
                  hintText: 'Search issue no…', hintStyle: const TextStyle(fontSize: 12),
                          ...
                          : Padding(
                              ...
                              child: Column(children: [
                                Container(
                                  ...
                                  child: Row(children: [
                                    _th('Issue No', flex: 2), _th('Date', flex: 2), _th('Location', flex: 3), _th('Status', flex: 2), _th('', flex: 1),
                                  ]),
                                ),
                                Expanded(
        ...
        if (!_loading)
          Padding(
            ...
            child: Text(_searchText.isNotEmpty ? '${rows.length} of ${_rows.length} issue(s)' : '${rows.length} issue(s)',
  ...
  Widget _emptyState() => const Center(
    child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
      Icon(Icons.outbox_outlined, size: 48, color: AppColors.textDisabled),
      SizedBox(height: 16),
      Text('No material issues found', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w600, color: AppColors.textPrimary)),
      SizedBox(height: 8),
      Text('Issue material against an approved Requisition to get started.', style: TextStyle(fontSize: 13, color: AppColors.textSecondary)),
    ]),
  );
}
