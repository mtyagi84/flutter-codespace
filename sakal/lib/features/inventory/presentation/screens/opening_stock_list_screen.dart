...
class _OpeningStockListScreenState extends ConsumerState<OpeningStockListScreen>
    with ScreenPermissionMixin<OpeningStockListScreen>, ScreenHeaderMixin<OpeningStockListScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      if (mounted) {
        setState(() {
          _rows       = results[0] as List<OpeningStockHeader>;
  ...
  List<OpeningStockHeader> get _filtered {
    if (_searchText.isEmpty) return _rows;
    return _rows.where((r) => r.openingNo.toLowerCase().contains(_searchText)).toList();
  }
  ...
  Future<void> _openEdit(OpeningStockHeader r) async {
    await context.push(RouteNames.openingStockEntry, extra: {'openingNo': r.openingNo, 'openingDate': r.openingDate});
    if (mounted) _load();
  }
  ...
  Widget build(BuildContext context) {
    ...
    // Title + New Opening Stock button live in the shared TopBar via
      ...
      children: [
        ...
        Expanded(
          child: SakalAdaptiveList(
            ...
            rows: rows,
            columns: const [
              SakalListColumn('Opening No', flex: 2),
  ...
  Widget _buildRow(OpeningStockHeader r, int index) {
    return InkWell(
      onTap: () => _openEdit(r),
      child: Container(
        ...
        child: Row(children: [
          Expanded(flex: 2, child: Padding(padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 12),
              child: Text(r.openingNo, overflow: TextOverflow.ellipsis,
          ...
          Expanded(flex: 2, child: Padding(padding: const EdgeInsets.symmetric(horizontal: 12),
              child: Text(_displayDate(r.openingDate), style: const TextStyle(fontSize: 13)))),
          ...
          Expanded(flex: 2, child: Padding(padding: const EdgeInsets.symmetric(horizontal: 12), child: Row(children: [
            ...
            if (_pendingIds.contains(r.openingNo)) ...[const SizedBox(width: 6), const PendingSyncBadge.static(isPending: true)],
          ]))),
  ...
  Widget _buildCard(OpeningStockHeader r) {
    return InkWell(
      onTap: () => _openEdit(r),
      ...
      child: Container(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(children: [
            Expanded(child: Text(r.openingNo, style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w600, color: AppColors.primary))),
            _statusBadge(r.status),
            if (_pendingIds.contains(r.openingNo)) ...[const SizedBox(width: 6), const PendingSyncBadge.static(isPending: true)],
          ]),
          const SizedBox(height: 6),
          Text(_displayDate(r.openingDate), style: const TextStyle(fontSize: 12, color: AppColors.textSecondary)),
