...
class StockCountReviewListScreen extends ConsumerStatefulWidget {
  const StockCountReviewListScreen({super.key});

  @override
  ConsumerState<StockCountReviewListScreen> createState() => _StockCountReviewListScreenState();
}
...
class _StockCountReviewListScreenState extends ConsumerState<StockCountReviewListScreen>
  ...
  Future<void> _load() async {
    ...
    try {
      final rows = await ref.read(stockCountReviewRepositoryProvider).listReviews(
        clientId: session.clientId, companyId: session.companyId, status: _filterStatus,
      );
      if (mounted) setState(() { _rows = rows; _loading = false; });
    } catch (e) {
      if (mounted) setState(() { _loading = false; _error = 'Could not load stock count reviews: $e'; });
    }
  }
  ...
  Future<void> _openNew() async {
    await context.push(RouteNames.stockCountReviewEntry);
    if (mounted) _load();
  }
          ...
          Padding(
            ...
            child: Text(
              _searchText.isNotEmpty
                  ? '${rows.length} of ${_rows.length} ${_rows.length == 1 ? 'review' : 'reviews'}'
  ...
  Widget _emptyState() => const Center(
    child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
      Icon(Icons.fact_check_outlined, size: 48, color: AppColors.textDisabled),
      SizedBox(height: 16),
      Text('No stock count reviews found', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w600, color: AppColors.textPrimary)),
      SizedBox(height: 8),
      Text('Club submitted Stock Counts together to review and post their variance.', style: TextStyle(fontSize: 13, color: AppColors.textSecondary)),
    ]),
  );
}
