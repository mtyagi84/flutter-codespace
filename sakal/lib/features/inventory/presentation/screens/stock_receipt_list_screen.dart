...
import '../../data/models/stock_receipt_model.dart';
...
class StockReceiptListScreen extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<StockReceiptListScreen> createState() => _StockReceiptListScreenState();
}

class _StockReceiptListScreenState extends ConsumerState<StockReceiptListScreen>
    with ScreenPermissionMixin<StockReceiptListScreen>, ScreenHeaderMixin<StockReceiptListScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      if (mounted) {
        setState(() {
          _rows       = results[0] as List<StockReceiptHeader>;
  ...
  List<StockReceiptHeader> get _filtered {
    if (_searchText.isEmpty) return _rows;
    return _rows.where((r) => r.receiptNo.toLowerCase().contains(_searchText)).toList();
  }

  Future<void> _openNew() async {
    await context.push(RouteNames.stockReceiptEntry);
    if (mounted) _load();
  }

  Future<void> _openEdit(StockReceiptHeader r) async {
    await context.push(RouteNames.stockReceiptEntry,
        extra: {'receiptNo': r.receiptNo, 'receiptDate': r.receiptDate});
    if (mounted) _load();
  }
    ...
    final statusField = SakalFieldCard(
      ...
      child: DropdownButtonFormField<String?>(
        ...
        items: [
          ...
          ..._statusColors.keys.map((s) => DropdownMenuItem(value: s, child: Text(s))),
        ],
      ...
      children: [
        ...
        Expanded(
          child: SakalAdaptiveList(
            ...
            columns: const [
              ...
              SakalListColumn('Source Transfer', flex: 2),
        ...
        if (!_loading)
          Padding(
            ...
            child: Text(
              _searchText.isNotEmpty ? '${rows.length} of ${_rows.length} receipt(s)' : '${rows.length} receipt(s)',
  ...
  Widget _buildRow(StockReceiptHeader r, int index) {
    return InkWell(
      onTap: () => _openEdit(r),
      child: Container(
        ...
        child: Row(children: [
          Expanded(flex: 2, child: Padding(padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 12),
              child: Text(r.receiptNo, overflow: TextOverflow.ellipsis,
          ...
          Expanded(flex: 2, child: Padding(padding: const EdgeInsets.symmetric(horizontal: 12),
              child: Text(_displayDate(r.receiptDate), style: const TextStyle(fontSize: 13)))),
          Expanded(flex: 2, child: Padding(padding: const EdgeInsets.symmetric(horizontal: 12),
          ...
          Expanded(flex: 2, child: Padding(padding: const EdgeInsets.symmetric(horizontal: 12),
              child: Text(r.sourceTransferNo ?? '—', maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(fontSize: 12, color: AppColors.textSecondary)))),
          Expanded(flex: 2, child: Padding(padding: const EdgeInsets.symmetric(horizontal: 12),
              child: Row(children: [
  ...
  Widget _buildCard(StockReceiptHeader r) {
    return InkWell(
      onTap: () => _openEdit(r),
      ...
      child: Container(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(children: [
            Expanded(child: Text(r.receiptNo,
          ...
          Text('${_displayDate(r.receiptDate)} · ${r.sourceTransferNo ?? '—'}', style: const TextStyle(fontSize: 12)),
        ]),
      ),
    );
  }
  ...
  Widget _emptyState() => const Center(
    child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
      Icon(Icons.move_to_inbox_outlined, size: 48, color: AppColors.textDisabled),
      SizedBox(height: 16),
      Text('No stock receipts found', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w600, color: AppColors.textPrimary)),
      SizedBox(height: 8),
      Text('Receive an approved Stock Transfer to get started.', style: TextStyle(fontSize: 13, color: AppColors.textSecondary)),
    ]),
  );
}
