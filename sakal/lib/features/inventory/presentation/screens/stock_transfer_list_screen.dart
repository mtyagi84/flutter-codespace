...
class _StockTransferListScreenState extends ConsumerState<StockTransferListScreen>
    with ScreenPermissionMixin<StockTransferListScreen>, ScreenHeaderMixin<StockTransferListScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      final results = await Future.wait([
        ref.read(stockTransferRepositoryProvider).listTransfers(
          clientId: session.clientId, companyId: session.companyId, status: _filterStatus,
        ),
        ref.read(syncEngineProvider).pendingDocumentIds('STOCK_TRANSFER'),
      ]);
      if (mounted) {
        setState(() {
      ...
      if (mounted) setState(() { _loading = false; _error = 'Could not load stock transfers: $e'; });
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    // Title + New Transfer button live in the shared TopBar via
      ...
      children: [
        ...
        Expanded(
          child: SakalAdaptiveList(
            ...
            rows: rows,
            columns: const [
              SakalListColumn('Transfer No', flex: 2),
        ...
        if (!_loading)
          Padding(
            ...
            child: Text(
              _searchText.isNotEmpty ? '${rows.length} of ${_rows.length} transfer(s)' : '${rows.length} transfer(s)',
  ...
  Widget _emptyState() => const Center(
    child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
      ...
      SizedBox(height: 16),
      Text('No stock transfers found', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w600, color: AppColors.textPrimary)),
      SizedBox(height: 8),
      Text('Raise a Stock Transfer to move stock between locations.', style: TextStyle(fontSize: 13, color: AppColors.textSecondary)),
    ]),
  );
}
