...
class _SupplierMasterScreenState extends ConsumerState<SupplierMasterScreen>
    with ScreenPermissionMixin<SupplierMasterScreen>, ScreenHeaderMixin<SupplierMasterScreen> {
  ...
  // no page-level title of its own, just panel-internal headers
  // ("Suppliers" list header, "New/Edit Supplier" form header). A plain
  // static title here is still better than TopBar's blank fallback.
  //
  // The Add button lives here (not in _listPanel()'s own header) so it's
  // hidden while a record is being added/edited — same fix as
  // customer_master_screen.dart's identical bug.
  @override
  ...
  // Real bug fixed 2026-09-13: a brand-new supplier's Ledger Currency used
  // to default to blank (NULL account_currency_id) -- same fix as
  // customer_master_screen.dart. Defaults to the company's base currency,
  // still freely changeable before the first save.
