...
class SalesOrderLocalDs {
  ...
  Future<void> cacheHeader(Map<String, dynamic> h) async {
    ...
    final paymentTerm     = h['payment_term'] as Map<String, dynamic>?;
    final incoterm        = h['incoterm'] as Map<String, dynamic>?;
    await _db.into(_db.salesOrdersCache).insertOnConflictUpdate(
          SalesOrdersCacheCompanion.insert(
            ...
            shipTo:              Value(h['ship_to'] as String? ?? ''),
            billTo:              Value(h['bill_to'] as String? ?? ''),
            expectedDeliveryDate: Value(h['expected_delivery_date'] as String? ?? ''),
            ...
            paymentTermId:       Value(h['payment_term_id'] as String? ?? ''),
            paymentTermName:     Value(paymentTerm?['term_name'] as String? ?? ''),
            incotermId:          Value(h['incoterm_id'] as String? ?? ''),
            incotermLabel:       Value(incoterm?['description'] as String? ?? ''),
            deliveryInstructions: Value(h['delivery_instructions'] as String? ?? ''),
  ...
  Future<void> cacheFromMaps(
    ...
    await _db.into(_db.salesOrdersCache).insertOnConflictUpdate(
          SalesOrdersCacheCompanion.insert(
            ...
            expectedDeliveryDate: Value(headerMap['expected_delivery_date'] as String? ?? ''),
            ...
            paymentTermId:       Value(headerMap['payment_term_id'] as String? ?? ''),
            incotermId:          Value(headerMap['incoterm_id'] as String? ?? ''),
            deliveryInstructions: Value(headerMap['delivery_instructions'] as String? ?? ''),
  ...
  Map<String, dynamic> _headerToMap(SalesOrderCacheEntry r) => {
        ...
        'ship_to':              r.shipTo,
        'bill_to':              r.billTo,
        'expected_delivery_date': r.expectedDeliveryDate.isEmpty ? null : r.expectedDeliveryDate,
        ...
        'payment_term_id':      r.paymentTermId.isEmpty ? null : r.paymentTermId,
        'payment_term':         {'term_name': r.paymentTermName},
        'incoterm_id':          r.incotermId.isEmpty ? null : r.incotermId,
        'incoterm':             {'description': r.incotermLabel},
