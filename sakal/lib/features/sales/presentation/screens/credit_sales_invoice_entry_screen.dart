...
class CreditSalesInvoiceEntryScreen extends ConsumerStatefulWidget {
  ...
  const CreditSalesInvoiceEntryScreen({
    super.key,
    this.editInvoiceNo,
  ...
  @override
  ConsumerState<CreditSalesInvoiceEntryScreen> createState() => _CreditSalesInvoiceEntryScreenState();
}

class _CreditSalesInvoiceEntryScreenState extends ConsumerState<CreditSalesInvoiceEntryScreen>
    with ScreenPermissionMixin<CreditSalesInvoiceEntryScreen>, ScreenHeaderMixin<CreditSalesInvoiceEntryScreen>, DeferredRowDisposal<CreditSalesInvoiceEntryScreen> {
  @override
  String get screenName => '/sales/credit-invoices';
  ...
  ScreenHeaderInfo buildScreenHeader() {
    ...
    //
    // Credit Sales Invoice deliberately splits Save Draft and Approve into
    // two separate actions (canAdd/canApprove) — unlike Quick Invoice's
    // single auto-approving Save — matching the standard document pattern
    // (PO/GRN/Sales Order) since a Credit Invoice can sit in DRAFT
    // indefinitely for later review/approval.
    final isOffline = ref.read(sessionProvider)?.offlineMode ?? false;
    final canSaveDraftNow = _status == 'DRAFT' && (_isNew ? canAdd : canEdit);
    final canApproveNow = !isOffline && _status == 'DRAFT' && !_isNew && canApprove;
    final showCancelNow = !isOffline && _status == 'DRAFT' && canApprove && !_isNew;
    final showDesktopActions = !Responsive.isMobile(context);
    return ScreenHeaderInfo(
      title: _invoiceNo != null ? 'Credit Sales Invoice · $_invoiceNo' : 'New Credit Sales Invoice',
          ...
          ? [
              ...
              if (canApproveNow)
                SakalHeaderActionButton(
                  ...
                  loading: _approving, onPressed: _approving ? null : _approveInvoice,
                ),
              if (showCancelNow)
                SakalHeaderActionButton(
  ...
  // Always CREDIT on this screen — Cash sales are Quick Invoice's own
  // domain (ric_user_quick_invoice_setup dependency, walk-in party
  // fields, immediate cash collection). Never toggled here.
  final String _saleType = 'CREDIT';
  String _status = 'DRAFT';
  bool get _isNew => _invoiceNo == null;
  bool get _isAgainstSource => _invoiceMode != 'DIRECT';
  // Once a Credit Sales Invoice has been saved once, its date is locked
  // — mirrors fn_save_sales_invoice's own new server-side
  // INVOICE_DATE_LOCKED_AFTER_FIRST_SAVE guard (migration 146), scoped
  // to this screen only via p_credit_invoice_screen.
  bool get _dateLocked => !_isNew;
  List<Map<String, dynamic>> _accessibleLocations = [];
  ...
  Future<void> _init() async {
    ...
    // No ric_user_quick_invoice_setup dependency at all on this screen —
    // Location and Customer are both operator-chosen (see below), and
    // stock always dispatches DEFERRED regardless of the company's own
    // quick_invoice_dispatch_stock flag (server-enforced via
    // p_credit_invoice_screen, migration 146).
    _dispatchStock = false;
    _collectCash   = false;
    ...
    try {
      final results = await Future.wait([
        ...
        ref.read(accessibleLocationsProvider.future),
      ]);
  ...
  // Save Draft ONLY — never chains into Approve, unlike Quick Invoice's
  // single auto-approving Save. Approve is a deliberate, separate action
  // (_approveInvoice below) matching this app's standard document
  // pattern (PO/GRN/Sales Order) — a Credit Invoice can sit in DRAFT
  // indefinitely for later review/approval.
  Future<bool> _saveDraft() async {
    ...
    // No cash-collection UI/validation on this screen at all — Credit
    // Sales Invoice never collects cash at save/approve time (collection
    // happens later via the Payment/Receipt Voucher screen, against this
    // invoice's own posted bill).
    ...
    try {
      ...
      if (session.offlineMode) {
        ...
        await ref.read(syncEngineProvider).enqueue(
          ...
          // p_credit_invoice_screen: true — Approve stays online-only for
          // this screen (matches every other module's convention), but
          // the offline-queued Save must still identify itself so a
          // future-dated or date-changed offline save is rejected once
          // synced, same as an online save would reject it immediately.
          payload: {
            'p_header': header, 'p_lines': lines, 'p_charges': charges, 'p_batches': batches, 'p_serials': serials,
            'p_user_id': session.userId, 'p_enforce_cost_check': false, 'p_credit_invoice_screen': true,
          },
        );
      ...
      final invoiceNo = await ds.save(
        header: header, lines: lines, charges: charges, batches: batches, serials: serials, userId: session.userId,
        creditInvoiceScreen: true,
      );
      await _loadExisting(invoiceNo, _fmtDate(_invoiceDate));
      if (mounted) {
        setState(() => _saving = false);
        _showSnack('Credit Sales Invoice $invoiceNo saved as Draft.', color: AppColors.positive);
      }
      return true;
    } catch (e, st) {
  ...
  // Separate, deliberate Approve action — never auto-chained from Save
  // (see this screen's own header comment on _saveDraft). Online-only,
  // same convention as every other module in this app.
  Future<bool> _approveInvoice() async {
    if (_approving || _invoiceNo == null) return false;
    ...
    try {
      ...
      if (mounted) {
        ...
        _showSnack('Credit Sales Invoice $_invoiceNo approved.', color: AppColors.positive);
      }
      return true;
    } catch (e, st) {
      AppLogger.error('SalesInvoiceApprove', e, st);
      setState(() { _approving = false; _actionError = ErrorPresenter.format(e, action: 'approve this invoice'); });
      return false;
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    final canSaveDraft = _status == 'DRAFT' && (_isNew ? canAdd : canEdit);
    final canApproveNow = !isOffline && _status == 'DRAFT' && !_isNew && canApprove;
    ...
    return Column(
      ...
      children: [
        ...
        if (isMobile && (canSaveDraft || canApproveNow || showCancel))
          Padding(
            padding: const EdgeInsets.fromLTRB(24, 16, 24, 8),
            child: _buildActionButtons(canSaveDraft: canSaveDraft, canApprove: canApproveNow, showCancel: showCancel),
          ),
        Expanded(
          child: _loading
  ...
  Widget _buildActionButtons({required bool canSaveDraft, required bool canApprove, required bool showCancel}) => Wrap(spacing: 12, runSpacing: 8, children: [
        if (canSaveDraft) FilledButton(
      ...
      child: Padding(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          SakalFieldRow(isMobile: isMobile, children: [
            // Location is a real, editable picker on this screen (unlike
            // Quick Invoice's read-only quick-setup location) — gated by
            // ric_user_location_access via accessibleLocationsProvider
            // (v_user_accessible_locations, migration 127), not the
            // unrestricted full-company list Sales Order's own dropdown
            // uses.
            SakalFieldCard(
              label: 'Location',
              required: true,
              ...
              child: DropdownButtonFormField<String>(
                // Externally set (session default at load, re-set on
                // mode-switch reset) as well as user-picked — needs a
                // ValueKey so a value change from OUTSIDE onChanged forces
                // a clean remount instead of silently keeping the stale
                // display (this app's own established FormField gotcha).
                key: ValueKey(_locationId),
                decoration: SakalFieldCard.bareDecoration,
                isExpanded: true, isDense: true, itemHeight: null,
                style: fieldTextStyle,
                initialValue: _locationId,
                items: _accessibleLocations
                    .map((l) => DropdownMenuItem(value: l['id'] as String, child: Text(l['location_name'] as String, overflow: TextOverflow.ellipsis)))
                    .toList(),
                onChanged: locked ? null : (v) => setState(() {
                  _locationId = v;
                  _locationName = _accessibleLocations.firstWhere((l) => l['id'] == v, orElse: () => const {})['location_name'] as String? ?? '';
                }),
              ),
            ),
            (!_isAgainstSource)
                ? SakalFieldCard(
                    ...
                    child: SakalAutocomplete<Map<String, dynamic>>(
                        ...
                        optionsBuilder: (v) async {
                          ...
                          // customer to invoice -- same fix as Sales Order/Quotation.
          ...
          SakalFieldRow(isMobile: isMobile, children: [
            // Editable up to the first save, then locked client-side
            // (mirrors fn_save_sales_invoice's own new server-side
            // INVOICE_DATE_LOCKED_AFTER_FIRST_SAVE guard) — lastDate=today
            // is a client-side UX guard for the new hard server-side
            // FUTURE_DATE_NOT_ALLOWED block.
            SakalFieldCard(
              label: 'Invoice Date',
              required: true,
              editable: !locked && !_dateLocked,
              child: InkWell(
                onTap: (locked || _dateLocked) ? null : () async {
                ...
                child: InputDecorator(
                  decoration: SakalFieldCard.bareDecoration.copyWith(
                    helperText: _dateLocked ? 'Locked after first save' : null,
                  ),
                  child: Text(_displayDate(_fmtDate(_invoiceDate)), style: fieldTextStyle),
                ),
              ),
            ),
            ...
            if (_invoiceCurrencyCode != null && _invoiceCurrencyCode != _baseCurrency)
              SakalFieldCard(
                ...
                editable: !locked && _canOverridePrice && !_isAgainstSource,
                child: TextFormField(
                  controller: _rateToBaseCtrl, enabled: !locked && _canOverridePrice && !_isAgainstSource,
            ...
            if (_invoiceCurrencyCode != null && _invoiceCurrencyCode != _localCurrency)
              SakalFieldCard(
                ...
                editable: !locked && _canOverridePrice && !_isAgainstSource,
                child: TextFormField(
                  controller: _rateToLocalCtrl, enabled: !locked && _canOverridePrice && !_isAgainstSource,
                  keyboardType: const TextInputType.numberWithOptions(decimal: true),
                  decoration: SakalFieldCard.bareDecoration,
                  style: fieldTextStyle,
                  onChanged: locked ? null : (_) => setState(() {}),
