...
class _PriceMasterListScreenState extends ConsumerState<PriceMasterListScreen>
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      AppLogger.error('PriceMasterListLoad', e, st);
      if (mounted) setState(() { _loading = false; _error = ErrorPresenter.format(e, action: 'load price batches'); });
    }
  }
        ...
        Padding(
          ...
          child: isMobile
              ...
              : Row(children: [
                  ...
                  const SizedBox(width: 8),
                  Expanded(flex: 4, child: locationField),
                  const SizedBox(width: 8),
                  Expanded(flex: 3, child: effFromField),
                  const SizedBox(width: 8),
                  Expanded(flex: 3, child: effToField),
                  if (clearDateButton != null) ...[const SizedBox(width: 4), clearDateButton],
                  const SizedBox(width: 8),
