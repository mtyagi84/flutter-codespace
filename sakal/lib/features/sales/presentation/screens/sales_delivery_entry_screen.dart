...
class _SalesDeliveryEntryScreenState extends ConsumerState<SalesDeliveryEntryScreen>
    with ScreenPermissionMixin<SalesDeliveryEntryScreen>, ScreenHeaderMixin<SalesDeliveryEntryScreen> {
  ...
  Future<void> _onInvoiceSelected(Map<String, dynamic> invoice) async {
    ...
    try {
      ...
      AppLogger.error('SalesDeliveryLoadInvoiceLines', e, st);
  ...
  Future<void> _loadCandidates(_SDLineRow row, {required bool autoAllocate}) async {
    ...
    try {
      ...
      AppLogger.error('SalesDeliveryLoadBatchSerial', e, st);
  ...
  Widget _buildHeaderCard(bool locked, bool isMobile) {
    ...
    final dateLocked = locked || _deliveryNo != null;
    ...
    final deliveryDateField = SakalFieldCard(
      ...
      child: InkWell(
        onTap: dateLocked ? null : () => _pickDate(_deliveryDate, (d) => setState(() => _deliveryDate = d)),
        child: Row(children: [
  ...
  Widget _buildLinesHeader(bool showLooseQty) {
    return SakalTableHeaderBar(cells: [
      ...
      SizedBox(width: 90, child: SakalTableHeaderBar.label('Pending')),
      const SizedBox(width: 8),
      SizedBox(width: 70, child: SakalTableHeaderBar.label('Unit')),
      const SizedBox(width: 8),
      SizedBox(width: 130, child: SakalTableHeaderBar.label('Barcode')),
      const SizedBox(width: 8),
      SizedBox(width: 110, child: SakalTableHeaderBar.label(showLooseQty ? 'Delivery Qty Pack' : 'Delivery Qty')),
      if (showLooseQty) ...[const SizedBox(width: 8), SizedBox(width: 110, child: SakalTableHeaderBar.label('Delivery Qty Loose'))],
