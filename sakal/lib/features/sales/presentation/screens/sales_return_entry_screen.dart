...
class _SalesReturnEntryScreenState extends ConsumerState<SalesReturnEntryScreen>
  ...
  Future<void> _init() async {
    ...
    try {
      ...
      AppLogger.error('SalesReturnLoadHeader', e, st);
      if (mounted) setState(() { _loading = false; _error = ErrorPresenter.format(e, action: 'load this sales return'); });
    }
  }
  ...
  Future<bool> _saveDraft() async {
    ...
    try {
      ...
      AppLogger.error('SalesReturnSave', e, st);
      setState(() { _saving = false; _actionError = ErrorPresenter.format(e, action: 'save this sales return'); });
      return false;
    }
  }
  ...
  Future<void> _approveReturn() async {
    ...
    try {
      ...
      AppLogger.error('SalesReturnApprove', e, st);
      setState(() { _actionError = ErrorPresenter.format(e, action: 'approve this sales return'); });
    } finally {
  ...
  Widget _buildHeaderCard(bool locked, bool isMobile) {
    ...
    final returnDateField = SakalFieldCard(
      ...
      child: InkWell(
        onTap: dateLocked ? null : () => _pickDate(_returnDate, (d) => setState(() => _returnDate = d)),
        child: Row(children: [
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
          SizedBox(width: 200, child: productField),
          const SizedBox(width: 8),
          SizedBox(width: 90, child: invoicedField),
          if (showAlreadyReturned) ...[const SizedBox(width: 8), SizedBox(width: 110, child: alreadyReturnedField)],
          const SizedBox(width: 8),
          SizedBox(width: 90, child: remainingField),
          const SizedBox(width: 8),
