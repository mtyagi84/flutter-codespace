...
class SalesReturnListScreen extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<SalesReturnListScreen> createState() => _SalesReturnListScreenState();
}

class _SalesReturnListScreenState extends ConsumerState<SalesReturnListScreen>
    with ScreenPermissionMixin<SalesReturnListScreen>, ScreenHeaderMixin<SalesReturnListScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      final results = await Future.wait([
        ref.read(salesReturnRepositoryProvider).listReturns(
          clientId: session.clientId, companyId: session.companyId,
          search: _searchText.isNotEmpty ? _searchText : null, status: _filterStatus,
        ),
        ref.read(syncEngineProvider).pendingDocumentIds('SALES_RETURN'),
      ]);
      if (mounted) {
        setState(() {
      ...
      if (mounted) setState(() { _loading = false; _error = 'Could not load sales returns: $e'; });
    }
  }
  ...
  Future<void> _openNew() async {
    await context.push(RouteNames.salesReturnEntry);
    if (mounted) _load();
  }
  ...
  Widget build(BuildContext context) {
    // Title + New Return button live in the shared TopBar via
    ...
    final searchField = SakalFieldCard(
      ...
      child: TextField(
        ...
        decoration: SakalFieldCard.bareDecoration.copyWith(
          hintText: 'Search return no / invoice no…',
      ...
      children: [
        ...
        Expanded(
          child: SakalAdaptiveList<SalesReturnHeader>(
            ...
            columns: const [
              ...
              SakalListColumn('Total', flex: 2, numeric: true),
        ...
        if (!_loading)
          Padding(
            ...
            child: Text('${rows.length} return(s)',
    ...
    return InkWell(
      ...
      child: Container(
        ...
        child: Row(children: [
          ...
          Expanded(flex: 2, child: Padding(
            padding: const EdgeInsets.symmetric(horizontal: 12),
            child: Text(total.toStringAsFixed(2), textAlign: TextAlign.right,
  ...
  Widget _emptyState() => const Center(
    child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
      ...
      SizedBox(height: 16),
      Text('No sales returns found',
          style: TextStyle(fontSize: 16, fontWeight: FontWeight.w600, color: AppColors.textPrimary)),
      SizedBox(height: 8),
      Text('Raise a Sales Return against an approved invoice to get started.',
