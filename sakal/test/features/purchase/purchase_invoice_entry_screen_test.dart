...
void main() {
  ...
  group('New (blank) invoice', () {
    testWidgets('renders the blank form with all key fields (consolidation-only screen, no product lines)', (tester) async {
      ...
      final header = _readHeader(tester, find.byType(PurchaseInvoiceEntryScreen));
      expect(header?.title, 'New Purchase Bill');
  ...
  group('Editing an existing DRAFT (resume flow — consolidated from GRN(s))', () {
    ...
    testWidgets('loads and displays every field from the saved header and pre-selected GRN', (tester) async {
      ...
      final header = _readHeader(tester, find.byType(PurchaseInvoiceEntryScreen));
      expect(header?.title, 'Purchase Bill · PINV-001');
  ...
  group('Supplier autocomplete interaction (real search + select)', () {
    ...
    testWidgets(
      ...
      final supplierOption = find.ancestor(of: find.text('[SUP-001] Test Supplier'), matching: find.byType(InkWell)).first;
      await tester.ensureVisible(supplierOption);
      await tester.pump();
      await tester.tap(supplierOption, warnIfMissed: false);
